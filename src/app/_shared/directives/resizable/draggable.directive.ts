import { Directive, AfterViewInit, OnDestroy, ElementRef, Renderer2, Input } from "@angular/core";
...
@Directive({
  selector: '[radDraggable]',
  exportAs: 'radDraggable'
})
export class RadDraggableDirective implements AfterViewInit, OnDestroy {
  constructor(private _elRef: ElementRef<HTMLElement>, private _rd: Renderer2) {
  }

  ngAfterViewInit(): void {
    this._dragHandlerHeight = (typeof this.handlerHeight === 'number')
  ...
  private _prepareDragEvent(parentEl: any, handlerEl: HTMLElement): void {
    ...
    this._subscription.add(
      this.mousedown$.pipe(
        map((mouseDownEvent) => {
          const { left, top } = this._elRef.nativeElement.getBoundingClientRect();
          return {
        ...
        switchMap(({ prevX, prevY, prevLeft, prevTop }) => this.mousemove$.pipe(
          tap(mouseMoveEvent => {
