...
import { BREAKPOINTS, isMediaMatched } from '../../common/utils';
import {
	Directions,
	RaxDirection,
	RaxResizeHandler,
...
export class RaxResizable implements AfterViewInit, OnDestroy {
	...
	private setHandlers(parentEl: HTMLElement): void {
		(this._handlerDirections.value as Array<RaxDirection>).forEach(
			(direction) => {
						...
						this.mousemove$.pipe(
							tap((event) => {
								const { height, width, top, left } =
									this.onDragStyle(direction, event);
